...
export default function FindMe(props: { data: { findme: { caption: string, adresses: Array<{ geo: string, address: string, city: string, cap: string, openings: string }>, email: string, phone: string, instagram: { username: string, url: string }, facebook: { username: string, url: string }, whatsapp: { number: string, url: string } } } }) {
  ...
  return (
    <div className="flex flex-col card p-4 w-full md:w-1/4">
      <h2 className="text-2xl text-center">{data.findme.caption}</h2>
        ...
        <div className="mt-4" key={i}>
          <a href={a.geo} target="_blank" rel="noopener noreferrer">
            <div className="flex items-center gap-2">
              <MapPin />
              <div>
                <h3 className="text-xl">{a.address}</h3>
                <h3 className="text-xl">{a.city}, {a.cap}</h3>
              </div>
            </div>
          </a>
          <p className="mt-2 ml-8 text-zinc-500">{a.openings}</p>
        </div>
      ))}
      <h4 className="mt-8 flex gap-2">
        <Mail />
        <a
